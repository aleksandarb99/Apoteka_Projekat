...
public interface RatingRepository extends JpaRepository<Rating, Long> {
    ...
    @Query("SELECT r FROM Rating r JOIN FETCH r.patient p WHERE r.gradedID = ?1 AND p.id = ?2 AND r.gradedType = 'PHARMACIST'")
    Rating findPharmacistRatingFetchPatient(Long pId, Long paId);

    @Query("SELECT r FROM Rating r JOIN FETCH r.patient p WHERE r.gradedID = ?1 AND p.id = ?2 AND r.gradedType = 'MEDICINE'")
    Rating findMedicineRatingFetchPatient(Long mId, Long paId);

    @Query("SELECT r FROM Rating r JOIN FETCH r.patient p WHERE r.gradedID = ?1 AND p.id = ?2 AND r.gradedType = 'PHARMACY'")
