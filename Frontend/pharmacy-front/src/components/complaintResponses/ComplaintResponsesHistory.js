...
const ComplaintResponsesHistory = () => {
    ...
    useEffect(() => {
        async function fetchComplaints() {
            api.get(`http://localhost:8080/api/complaint-responses/admin/${getIdFromToken()}`)
                .then((res) => {
    ...
    return (
        <div>
            {complaintResponses && complaintResponses.map((cr) => {
                return <ComplaintResponseDetails key={cr.id} complaintResponse={cr}></ComplaintResponseDetails>
            })}
        </div>
    )
}
